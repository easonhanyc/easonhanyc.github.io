...
| Party size | Given any join or rider post, when the traveller isn't alone, then they can say how many people they're accounting for, including anyone without a Berkeley account. A driver's remaining seats is the sum of party sizes rather than a count of claims, capacity is enforced against that sum, and rosters render a guest as "Name +1". A driver post has no party field — its seat count already states the places free for other people. |
